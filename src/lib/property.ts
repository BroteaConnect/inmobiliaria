...
export interface Property extends Propiedades { collectionId: string }
...
/** The photo count, tolerant of a record with no `photos` at all. */
export const photos = (p: Pick<Property, 'fotos'>): string[] => p.fotos ?? [];

/** The URL of the n-th photo, at strip size when asked; empty when there is none. */
export function photoUrl(p: Pick<Property, 'id' | 'collectionId' | 'fotos'>, i = 0, thumb = false): string {
  const f = photos(p)[i];
 ...
 * word, never mid-word.
 */
export function summaryOf(p: Pick<Property, 'descripcion'>, max = 160): string {
...
export const paragraphsOf = (p: Pick<Property, 'descripcion'>): string[] =>
  (p.descripcion ?? '').split(/\n+/).map((s) => s.trim()).filter(Boolean);

/** True when the description is the note and nothing more: printing both would print it twice. */
export function descriptionIsNote(p: Pick<Property, 'descripcion'>): boolean {
...
export const realPrice = (p: Pick<Property, 'precio'>): number =>
...
export function priceOf(locale: Locale, p: Pick<Property, 'precio'>, moneda = DEFAULT_CURRENCY): string {
  const n = realPrice(p);
 ...
 */
export function metaOf(locale: Locale, p: Pick<Property, 'habitaciones' | 'banos' | 'superficie'>): string {
