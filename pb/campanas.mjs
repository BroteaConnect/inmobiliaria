...
// instance; an unknown clave fails the seed before any write.
// Exit codes: 0 ok, 1 instance/credentials error, 2 catalog invalid, 64 usage.
import { readFileSync, existsSync } from 'node:fs';
import { join } from 'node:path';
import { homedir } from 'node:os';
import { catalogProblems } from '../jobs/campanas.lib.mjs';
...
const list = async (col) => (await call(`/api/collections/${col}/records?perPage=500`, { headers: H })).items ?? [];

const existing = new Set((await list('campanas')).map((c) => c.nombre));
...
for (const row of rows) {
  if (existing.has(row.nombre)) { console.log(`campanas: ${row.nombre} kept${dryRun ? ' (dry-run)' : ''}`); continue; }
